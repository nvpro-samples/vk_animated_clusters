...
public:
  ...
  AnimatedClusters::CallBacks   m_callbacks;
  AnimatedClusters::EventStates m_eventStates;
  AnimatedClusters              m_core;
  uint32_t                      m_width;
  uint32_t                      m_height;
...
public:
  ...
  void onUIRender() override
  {
    ...
    AnimatedClusters::ChangeStates changes = m_core.handleChanges(m_width, m_height, m_eventStates);
...
using namespace animatedclusters;
...
int main(int argc, char** argv)
{
  try
  {
    ...
    AnimatedClusters::setupContextInfo(contextCreateInfo);
...

    {
      ...
      auto elementSample    = std::make_shared<AnimatedClustersElement>();
      ...
      elementSample->m_profilerVK = elementProfiler;
      elementSample->setParameterList(elementBenchmark->parameterLists());
      elementSample->setContext(&context);

      app->addElement(elementBenchmark);
      app->addElement(elementSample);
