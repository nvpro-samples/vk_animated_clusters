...
void Resources::blitFrame(VkCommandBuffer cmd, const FrameConfig& frame, nvvk::ProfilerVK& profiler)
{
...

  {
    ...
    vkCmdPipelineBarrier(cmd, m_supportedSaderStageFlags, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 1, &barrier, 0, nullptr, 0, nullptr);
...
bool Resources::init(nvvk::Context* context, const std::vector<std::string>& shaderSearchPaths)
{
  ...
  {
    // common
    m_common.view = createBuffer(sizeof(shaderio::FrameConstants), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
...
RLargeBuffer Resources::createLargeBuffer(VkDeviceSize size, VkBufferUsageFlags flags, VkMemoryPropertyFlags memFlags)
{
